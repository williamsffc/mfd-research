...
export default function Slide03FeatureAdoption() {
  ...
  const totalAdoption = features
    .filter(f => selectedFeatures.includes(f.id))
    .reduce((sum, f) => sum + f.adoption, 0) / Math.max(selectedFeatures.length, 1);

  const totalUsers = features
    .filter(f => selectedFeatures.includes(f.id))
    .reduce((sum, f) => sum + parseFloat(f.users.replace('k', '')) * (f.users.includes('k') ? 1000 : 1), 0);
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 flex-1">
              ...
              <div className="space-y-2">
                ...
                {selectedFeatures.length === 0 ? (
                  <p className="text-sm text-slate-400 italic">Click features to select</p>
                ) : (
